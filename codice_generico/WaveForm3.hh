...
private:
 ...
 public:
  WaveForm(UShort_t*,Int_t,Int_t,Int_t,Int_t,Int_t,Int_t,
           Int_t,Int_t,Int_t,Int_t,Int_t,Int_t,Bool_t,Bool_t=kTRUE);
  // ~WaveForm();
...
WaveForm::WaveForm(UShort_t* array,Int_t size,Int_t nPoints,Int_t nWord,
                   Int_t thUp,Int_t thDown,Int_t nTime,Int_t channel,
                   Int_t module, Int_t digiBaseline,Int_t headerTot,
                   Int_t headerChan,Int_t trailerChan, Bool_t debug, Bool_t analyze) {
  ...
  for(Int_t i=0; i < nSample; i++) {
    ...
    waveVector.push_back(digiBaseline-Int_t(*((array+module*nWord)+index)));
  }
...
// WaveForm::~WaveForm() {
//   for(Int_t i=0; i<Int_t(lineStart.size()); i++)
//     delete lineStart.at(i);

//   for(Int_t i=0; i<Int_t(lineStop.size()); i++)
...
void WaveForm::CreateDeltaGraph() {
  ...
  for (Int_t i = 0; i < Int_t(deltaVec.size()); i++)
